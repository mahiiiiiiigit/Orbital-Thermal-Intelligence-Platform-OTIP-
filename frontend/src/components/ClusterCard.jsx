...
export function ClusterCard({ cluster, mode = 'demo', onSelectCluster }) {
  if (!cluster) {
    return (
      <div className="bg-dark-850/80 border border-slate-800/80 rounded-xl p-4 text-center">
        <Factory className="w-8 h-8 text-slate-600 mx-auto mb-2 opacity-50" />
        <h3 className="text-sm font-semibold text-slate-400">No Cluster Selected</h3>
  ...
  return (
    <div className="bg-dark-850/90 border border-slate-800 rounded-xl p-4 space-y-3 shadow-xl backdrop-blur-md">
      ...
      <div className="grid grid-cols-2 gap-2 text-xs">
        <div className="bg-dark-900/80 border border-slate-800/60 rounded-lg p-2">
          <span className="text-[10px] text-slate-400 uppercase tracking-wider block">Peak Radiance</span>
          <span className="text-sm font-bold text-amber-400 font-mono">{cluster.peak_frp} MW</span>
        </div>
        <div className="bg-dark-900/80 border border-slate-800/60 rounded-lg p-2">
          <span className="text-[10px] text-slate-400 uppercase tracking-wider block">Baseline Mean</span>
      ...
      {cluster.reasons && cluster.reasons.length > 0 && (
        <div className="text-[11px] text-slate-400 bg-dark-900/60 rounded-lg p-2">
          <span className="font-semibold text-slate-300">Cluster Rationale: </span>
          {cluster.reasons[0]}
        </div>
      )}
