...
export function Sidebar({
  ...
  return (
    <aside className="w-96 bg-dark-900/95 border-r border-slate-800/80 flex flex-col h-[calc(100vh-4rem)] select-none overflow-hidden z-20">
      {/* Scrollable Container */}
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {/* Stream Telemetry Status Notice */}
        {notice && (
          <div className="bg-sky-950/40 border border-sky-800/40 rounded-xl p-3 flex items-start gap-2.5 text-xs text-sky-200">
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className="bg-dark-850/90 border border-slate-800 rounded-xl p-3">
            <div className="flex items-center justify-between text-slate-400 mb-1">
              <span className="text-[10px] uppercase font-bold tracking-wider">Hotspots</span>
              <Flame className="w-3.5 h-3.5 text-sky-400" />
            </div>
            <span className="text-xl font-extrabold text-white font-mono">{stats.totalHotspots}</span>
            <span className="text-[10px] text-slate-400 block mt-0.5">Orbital Detections</span>
          </div>
          ...
          <div className="bg-dark-850/90 border border-slate-800 rounded-xl p-3">
            ...
            <span className="text-xl font-extrabold text-white font-mono">{stats.totalClusters}</span>
            <span className="text-[10px] text-slate-400 block mt-0.5">Persistent Sources</span>
          </div>
          ...
          <div className="bg-dark-850/90 border border-slate-800 rounded-xl p-3">
            ...
            <span className="text-xl font-extrabold text-red-400 font-mono">{stats.totalAlerts}</span>
            <span className="text-[10px] text-slate-400 block mt-0.5">&gt;3σ Critical Alerts</span>
          </div>
          ...
          <div className="bg-dark-850/90 border border-slate-800 rounded-xl p-3">
            ...
            <span className="text-xl font-extrabold text-amber-300 font-mono">{stats.avgFrp?.toFixed(1)}</span>
            <span className="text-[10px] text-slate-400 block mt-0.5">Megawatts (MW)</span>
          </div>
        </div>

        {/* 7-Class Taxonomy Filter & Legend */}
        <div className="bg-dark-850/90 border border-slate-800 rounded-xl p-3.5 space-y-2">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] text-slate-400 font-mono">7 SIH Classes</span>
          </div>
              ...
              return (
                <button
                  ...
                  className={`px-2.5 py-1 rounded-lg text-xs font-medium flex items-center gap-1.5 transition-all ${
                    isActive
                      ? 'bg-sky-600/30 text-white border border-sky-500/60 shadow-sm'
                      : 'bg-dark-900 text-slate-400 hover:text-slate-200 border border-slate-800/80 hover:border-slate-700'
                  }`}
                >
                  {cls.id !== 'all' && (
                  ...
                  <span>{cls.label}</span>
                  <span className="font-mono text-[10px] opacity-70 bg-dark-900/80 px-1 py-0.2 rounded">
                    {count}
                  </span>
                </button>
              );
        ...
        <div className="bg-dark-850/90 border border-slate-800 rounded-xl p-3.5 space-y-2">
          <div className="flex items-center justify-between text-xs font-bold text-slate-300 uppercase tracking-wider">
            ...
            <span className="text-[10px] text-sky-400 font-mono">MW Mean</span>
          </div>
