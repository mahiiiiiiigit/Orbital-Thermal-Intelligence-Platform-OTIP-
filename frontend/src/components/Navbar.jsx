...
import { Radio, RefreshCw, Layers, MapPin, Satellite, Flame, ShieldAlert, Activity, Sun, Moon, Trees, Shield, LifeBuoy } from 'lucide-react';
...
export function Navbar({
  ...
  return (
    <header className="h-16 bg-white/95 dark:bg-dark-850/95 border-b border-slate-200 dark:border-slate-800/80 px-4 flex items-center justify-between gap-4 z-30 select-none shadow-sm dark:shadow-md backdrop-blur-md transition-colors duration-200">
      ...
      {/* Center Controls: Scope, Sensor, Safety Layer Toggle */}
      <div className="hidden lg:flex items-center gap-2.5">
        ...
        {/* Emergency Resources Layer Toggle */}
        <button
          type="button"
          onClick={onToggleSafetyResources}
          className={`px-2.5 py-1 rounded-lg border text-xs font-semibold flex items-center gap-1.5 transition-all shadow-sm ${
            showSafetyResources
              ...
              : 'bg-slate-100 dark:bg-dark-900 border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
          }`}
          title="Toggle Emergency Safety Resources layer (Fire, Hospital, Police, EMS, Shelters)"
        >
          <Shield className="w-3.5 h-3.5 text-rose-500" />
          <span>Emergency Resources</span>
          <span className={`w-1.5 h-1.5 rounded-full ${showSafetyResources ? 'bg-rose-500' : 'bg-slate-400'}`} />
        </button>
